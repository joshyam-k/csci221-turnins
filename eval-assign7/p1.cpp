...
bool move_via_gas_suffix(car c, gasStation* station_array, int gsarray_len, int* success_index, double x, double y){
    bool found = false;
    ...
    *success_index = 0;
    for(int i = 0; i < gsarray_len; i++){
        ...
        for(int j = i; j < gsarray_len; j++){
            ...
            copy.setcurrent_fuel(copy.gettank_size());
            moves++;
...
struct return_mv2 which_can_move2(car* car_array, gasStation* station_array, int carray_len, int gsarray_len, int* return_length,  double x, double y){
    int n_can_move = 0;
    ...
    for(int i = 0; i < carray_len; i++){
        ...
        if(cartest.move_to(x, y) == true) {
            ...
            int iter_dummy = 0;
            if(move_via_gas_suffix(cartest, station_array, gsarray_len, &iter_dummy, x, y) == true) {
                n_can_move++;
            }
        }
    }
    *return_length = n_can_move;
    ...
    for(int i = 0, j = 0; i < carray_len; i++){
        ...
        if(carcheck.move_to(x,y) == true) {
            ...
            cost_array[j] = 0;
            j++;
        } else if(move_via_gas_suffix(carcheck, station_array, gsarray_len, &success_index, x, y ) == true){
